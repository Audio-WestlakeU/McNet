...
def get_magnitude_ratio_mask(noisy_complex_tensor,clean_complex_tensor,ref_channel_index):
    if noisy_complex_tensor.dim() != clean_complex_tensor.dim():
        noisy_complex_tensor = noisy_complex_tensor[:, ref_channel_index, ...]
    assert noisy_complex_tensor.dim() == clean_complex_tensor.dim()

    noisy_mag = (noisy_complex_tensor.pow(2.).sum(-1) + 1e-8).pow(0.5 * 1.0)
    clean_mag = (clean_complex_tensor.pow(2.).sum(-1) + 1e-8).pow(0.5 * 1.0)
    mask_temp = clean_mag / noisy_mag
    magnitude_ratio_mask = 1.0 * (mask_temp >= 1.0) + mask_temp * (mask_temp < 1.0)
    return magnitude_ratio_mask
...
def stft(y, n_fft, hop_length, win_length, device):
    if y.dim() == 3:
        ...
        audio_length = y.shape[2]
        y = y.reshape((batch_size * num_channel,audio_length))
        assert y.dim() == 2
        window = torch.hann_window(n_fft).to(device)
        y_stft = torch.stft(y, n_fft, hop_length, win_length, window=window, return_complex=False)
        num_frequency = y_stft.shape[1]
        num_frame = y_stft.shape[2]
        y_stft = y_stft.reshape((batch_size,num_channel,num_frequency,num_frame,2))
    else:
        assert y.dim() == 2
        window = torch.hann_window(n_fft).to(device)
        y_stft = torch.stft(y, n_fft, hop_length, win_length, window=window, return_complex=False)
...
def istft(complex_tensor, n_fft, hop_length, win_length, device, length=None, use_mag_phase=False):
    window = torch.hann_window(n_fft).to(device)
    if complex_tensor.dim() == 5:
        batch_size = complex_tensor.shape[0]
        num_channel = complex_tensor.shape[1]
        frequency_length = complex_tensor.shape[2]
        frame_length = complex_tensor.shape[3]
        complex_tensor = complex_tensor.reshape((batch_size * num_channel,frequency_length,frame_length,2))
    assert complex_tensor.dim() == 4
    if use_mag_phase:
...
def subsample(data,sub_sample_length,nchannels):
    ...
    Notes
        仅支持 1D 数据
    """
    assert np.ndim(data) == 2, f"Only support 1D data. The dim is {np.ndim(data)}"
    length = data.shape[1]
    ...
    if length > sub_sample_length:
        ...
        data = np.append(data, np.zeros((nchannels,sub_sample_length - length), dtype=np.float32),axis=1)
        return data
    else:
        return data
